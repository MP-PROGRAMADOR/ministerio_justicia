...
--------------------------- */
        function formatDate(dateString) {
            ...
            // Usamos el formato local o el de tu preferencia
        ...
        async function loadEmployeeData(funcionarioId) {
            ...
            modalContentData.innerHTML = ''; // Limpiar contenido anterior

            try {
                // Simulación de una carga de 1.5s
                ...
                <div class="col-12 text-center alert alert-danger border-0">
                ...
                // Función auxiliar para renderizar items de lista
                const renderListItems = (items, renderer, emptyMessage) => {
                    return items.length ?
                        items.map(item => `<li class="list-group-item border-0 p-0 mb-3">${renderer(item)}</li>`).join('') :
                        `<p class="text-muted fst-italic">${emptyMessage}</p>`;
                };
        ...
        <div class="col-md-4">
            <div class="card detail-card h-100 text-center p-4">
                
                <div class="mb-4">
                    <img src="${funcionario.Foto ?? 'https://placehold.co/150x150/0d6efd/ffffff?text=Perfil'}"
                        class="rounded-circle profile-img" alt="Foto de perfil"
                        style="width:150px;height:150px;object-fit:cover; margin: 0 auto;">
                </div>

                <h4 class="fw-bold mb-1">${funcionario.Nombre} ${funcionario.Apellidos}</h4>
                <p class="text-secondary small mb-3">DIP/Pasaporte: **${funcionario.Dip_Pasaporte}**</p>

                <span class="badge ${funcionario.Estado_Laboral === 'Activo' ? 'bg-success' : 'bg-danger'} fw-normal rounded-pill p-2 mb-4">
                    <i class="bi bi-briefcase-fill me-1"></i> ${funcionario.Estado_Laboral}
                </span>

                <ul class="list-group list-group-flush text-start small">
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <i class="bi bi-envelope-fill text-primary me-2"></i> Email:
                        <span class="fw-bold">${funcionario.Correo ?? 'N/A'}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <i class="bi bi-phone-fill text-primary me-2"></i> Teléfono:
                        <span class="fw-bold">${funcionario.Telefono ?? 'N/A'}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <i class="bi bi-calendar-check-fill text-primary me-2"></i> Nombrado:
                        <span class="fw-bold">${formatDate(funcionario.Fecha_nombramiento)}</span>
                    </li>
                    <li class="list-group-item d-flex justify-content-between align-items-center">
                        <i class="bi bi-house-door-fill text-primary me-2"></i> Domicilio:
                        <span class="text-muted text-end">${funcionario.Domicilio ?? 'N/A'}</span>
                    </li>
                </ul>
            </div>
        </div>
            ...
            <div class="card detail-card h-100 p-4">
                ...
                <div class="mb-4">
                    <h5 class="section-header text-primary">
                        <i class="bi bi-briefcase-fill"></i> Asignaciones
                    </h5>
                    <ul class="list-group list-group-flush">
                        ${renderListItems(asignaciones, (a) => `
                            <div class="fw-bold">${a.Nombre_Cargo}</div>
                            <small class="text-muted">${a.Nombre_Departamento} | ${a.Nombre_Destino}</small><br>
                            <small class="badge bg-light text-dark">${formatDate(a.Fecha_Inicio_Asignacion)}</small>
                ...
                <div>
                    <h5 class="section-header text-success">
                        <i class="bi bi-mortarboard-fill"></i> Formación Académica
                    </h5>
                    <ul class="list-group list-group-flush">
                        ${renderListItems(formacion_academica, (f) => `
                            <div class="fw-bold">${f.Titulo_Obtenido}</div>
                            <small class="text-muted">${f.Institucion_Educativa}</small><br>
                            <small class="badge bg-light text-dark">${formatDate(f.Fecha_Graduacion)}</small>
            ...
            <div class="card detail-card h-100 p-4">
                ...
                <div class="mb-4">
                    <h5 class="section-header text-info">
                        <i class="bi bi-lightbulb-fill"></i> Capacitaciones
                    </h5>
                    <ul class="list-group list-group-flush">
                        ${renderListItems(capacitaciones, (c) => `
                            <div class="fw-bold">${c.Nombre_Curso}</div>
                            <small class="text-muted">${c.Institucion_Organizadora}</small><br>
                            <small class="text-muted">${formatDate(c.Fecha_Inicio_Curso)}</small>
                            ${c.Certificado_URL ? `<br><a href="${c.Certificado_URL}" target="_blank" class="text-decoration-none small"><i class="bi bi-box-arrow-up-right"></i> Certificado</a>` : ''}
                        `, 'Sin capacitaciones registradas.')}
                    </ul>
                </div>

                <div>
                    <h5 class="section-header text-danger">
                        <i class="bi bi-calendar-x-fill"></i> Permisos Solicitados
                    </h5>
                    <ul class="list-group list-group-flush">
                        ${renderListItems(permisos, (p) => {
                            const badgeClass = p.Estado_Permiso === 'Aprobado' ? 'bg-success' : (p.Estado_Permiso === 'Pendiente' ? 'bg-warning text-dark' : 'bg-danger');
                            return `
                                <div class="fw-bold">${p.Tipo_Permiso}</div>
                                <small class="text-muted">${formatDate(p.Fecha_Inicio_Permiso)} - ${formatDate(p.Fecha_Fin_Permiso)}</small><br>
                                <span class="badge ${badgeClass} fw-normal">${p.Estado_Permiso}</span>
                                ${p.Documento_Soporte_URL ? `<br><a href="${p.Documento_Soporte_URL}" target="_blank" class="text-decoration-none small"><i class="bi bi-box-arrow-up-right"></i> Documento</a>` : ''}
                            `;
        ...
        `;
                ...
                lastVerificationDate.textContent = new Date().toLocaleDateString('es-ES', {
                    day: '2-digit',
                    month: 'short',
                    year: 'numeric',
                    hour: '2-digit',
                ...
                modalContentData.innerHTML = `
            <div class="col-12 text-center alert alert-danger border-0">
                <i class="bi bi-exclamation-triangle-fill me-2"></i> Error de conexión: ${error.message}
            </div>`;
            } finally {
                // 2. Ocultar Spinner
